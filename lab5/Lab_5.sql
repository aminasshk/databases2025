...
--    UNIQUE also ensures all values are unique but allows one NULL.
--    A table can have only one PRIMARY KEY but several UNIQUE constraints.

-- 2. Single-column vs Composite PRIMARY KEY:
--    Single-column key is used when one column can uniquely identify rows (e.g., user_id).
--    Composite key is used when only a combination of columns is unique (e.g., student_id + course_id).

-- 3. Why only one PRIMARY KEY but multiple UNIQUE constraints:
--    Because the PRIMARY KEY represents the table’s main unique identifier.
--    UNIQUE constraints just add extra uniqueness conditions on other columns.
